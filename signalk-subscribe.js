...
export default function(RED) {
  function input(config) {
              ...
              update.values.forEach(pathValue => {
                ...
                if ( typeof last === 'undefined'
                  ...
                  node.send({
                    ...
                    source: update.source,
                    context: delta.context == this.self ? 'vessels.self' : delta.context,
    ...
    const onConnect = () => {
      ...
      this.server.client.subscribe(subscription)
    }
    this.server.client.on('connect', onConnect)
    
    this.server.client.on('delta', on_delta)

    const onSelf = self => {
      debug('self updated', self)
      node.self = self
    }
    this.server.client.on('self', onSelf)
    node.on('close', function() {
      this.server.client.removeListener('delta', on_delta)
      this.server.client.removeListener('connect', onConnect)
      this.server.client.removeListener('self', onSelf)
    })
